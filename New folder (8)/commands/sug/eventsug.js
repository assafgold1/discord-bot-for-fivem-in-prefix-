...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (!suggestion) return message.reply({ content: `בבקשה תרשום הצעת איוונט!`});
        ...
        const msg = await channel.send({
            embeds: [new MessageEmbed()
                ...
                .setAuthor({ name: 'TheRockets | Event Suggest System', iconURL: message.author.displayAvatarURL({ dynamic: true }) })
                .addField('User', `${message.author} `)
                .addField(' Event Suggestion', suggestion, true)
                .addField('Event Suggestion Status', `YES = 0 | NO = 0`, true)
